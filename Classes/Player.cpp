...
void CPlayer::initData()
{
	...
	m_nDiedTime = 0;
	m_bOldState = 0;
	m_nInvincibleTime = 0; //变大后又变小的话会给大概3s（200帧）的无敌时间。防止由大变小后下一帧碰到敌人立马died
	m_bIsOpenTimer = false; // 是否开启无敌时间倒计时
...
void CPlayer::update(float delta)
{
	if (E_STATE_DIED == m_nState)
	{
		...
		{
			m_vVelocity.y += m_fAccel * delta * m_vDir.y;
			if (m_vVelocity.y <=0)
			{
				m_vDir.y = -1;
				m_vVelocity.y = 0;
			...
			fY += m_vDir.y * m_vVelocity.y * delta;
			if (fY <= 2)
			{
				fY = 2;
	...
	if (m_bIsOpenTimer && m_nInvincibleTime > 0)
	{
		m_nInvincibleTime--;
	}
}
...
string CPlayer::checkTouchY()
{
	...
	else
	{
		...
		{
			touchPoint = this->getTouchPoint(Vec2(0, 0));
		}
		else
		{
			touchPoint = this->getTouchPoint(Vec2(0, -1));
...
//small 14*16, big 18*32
void CPlayer::turnBig()
{
	m_bIsBig = true;
	this->setPositionY(this->getPositionY() + 8);
}

void CPlayer::turnSmall() 
{
	m_bIsBig = false;
	this->setPositionY(this->getPositionY() - 8);
}
...
{
	if (nState == m_nState && m_bOldState == m_bIsBig)
	{
		return;
	}
	m_nState = nState;
	m_bOldState = !m_bOldState;
	...
	if (E_STATE_JUMP == m_nState || E_STATE_FALL == m_nState)
	{
		...
		{
			m_pImage->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName("big_1_walk_11.png"));
		}
	}
	else if (E_STATE_WALK == m_nState)
	{
		...
		{
			m_aniWalk = Animate::create(AnimationCache::getInstance()->getAnimation("walk_b"));
		}
	...
	else if (E_STATE_IDLE == m_nState)
	{
		...
		{
			m_pImage->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName("big_1_walk_01.png"));
		}
	...
	else if (E_STATE_DIED == m_nState)
	{
		if (!m_bIsBig)
		{
			m_pImage->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName("small_1_die_02.png"));
		}
		else
		{
			m_pImage->setSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName("big_1_die_02.png"));
		}
	}
}
...
void CPlayer::updatePosition(float delta)
{
	...
	{
		Vec2 vGroundTile = CGameScene::getInstance()->getTileMap()->getTileByPos(Vec2(fPositionX + m_pImage->getContentSize().width/2 * m_vDir.x, this->getPositionY()));
	...
	else if (m_vDir.y == 1 && strTouchTypeY != "none" && strTouchTypeY != "coin")
	{
		m_vVelocity.y = 0;
		m_vDir.y = -1;
		Vec2 vTouchTile = CGameScene::getInstance()->getTileMap()->getTileByPos(Vec2(this->getPositionX(), fPositionY + m_pImage->getContentSize().height/2));

...
void CMushroom::createAnimations()
{
...
void CMushroom::changeState(int nState)
{
	if (nState == m_nState)
	{
		return;
...
void CMushroom::checkDistaceWithMario()
{
	m_pPlayer = CGameScene::getInstance()->getCPlayer();
	
	float fMushroomX =  m_vBirthPoint.x;
	float fPlayerX = m_pPlayer->getPositionX();
	if ((fMushroomX - fPlayerX) <= 180)
	{
		m_vDir.x = -1;
	}
}

void CMushroom::setBirthPoint(Vec2 m_vBirthPoint)
{
	this->m_vBirthPoint = m_vBirthPoint;
}

void CMushroom::updateDiedY(float delta)
{
